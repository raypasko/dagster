...
from dagster import Failure, dagster_type, execute_pipeline, solid, pipeline
...
def less_simple_data_frame_type_check(value):
    ...
    if not isinstance(value, list):
        raise Failure(
            'LessSimpleDataFrame should be a list of OrderedDicts, got '
    ...
    for i in range(len(value)):
        ...
        if not isinstance(row, OrderedDict):
            raise Failure(
                'LessSimpleDataFrame should be a list of OrderedDicts, '
...
if __name__ == '__main__':
    execute_pipeline(
        ...
        {'solids': {'bad_read_csv': {'inputs': {'csv_path': {'value': 'cereal.csv'}}}}}
    )
