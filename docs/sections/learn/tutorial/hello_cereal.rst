Hello, cereal!
---------------
We'll write our first Dagster pipelines around a simple but scary .csv dataset, ``cereal.csv``,
which contains nutritional facts about 80 breakfast cereals. You can find this dataset on
...
Let's write our first Dagster solid and save it as ``hello_cereal.py``. (You can also find this file, and all of the
tutorial code, on `Github <https://github.com/dagster-io/dagster/tree/master/examples/dagster_examples/intro_tutorial>`_
...
The logic in our first solid is super simple: it just reads in the csv from a hardcoded path
...
On line 19, you'll see that we call ``hello_cereal()``. This call doesn't actually execute the solid
...
You'll see a lot of log spew to the console, but somewhere in there will be lines like:
...
open, and you'll start to see structured logs stream into the left-hand pane. (If you have pop-up
blocking enabled, you may need to tell your browser to allow pop-ups from 127.0.0.1 -- or, just
navigate to the "Runs" tab to see this, and every run of your pipeline.)
...
The :py:func:`execute_pipeline <dagster.execute_pipeline>` function called on line 25 is the core
